...
export const Cart = () => {
    ...
    const decreaceProductAmount = (id) => {
        ...
        const changedOrderAmount = currentOrder.map(elem => {
            console.log('elem amount 1', elem.amount);
            if(elem.id === id && elem.amount !== 0){
                elem.amount -= 1
                console.log('elem amount 2', elem.amount);
            }
            console.log('elem amount 3', elem.amount);
            if(elem.amount === 0) {
                console.log('elem amount 4', elem.amount);
                // dispatch(removeProductFromCart(id))
                handleDelete()
            }
                        ...
                        currentOrder.map((order, i) => (
                            <div key={i} className={styles.cart_item}>
                                ...
                                <div className={styles.cart_item_info}>
                                    ...
                                    {/* < AmountCounter amount={order.amount} /> */}
                ...
                <div className={styles.total_orderinfo}>
                    {currentOrder.length ? (
                        <>
                            ...
                            <div className="button" onClick={() => dispatch(cleanCart())}>Delete order</div>
                        </>
                    ) : (
                        <h2>total price: 0 ₽</h2>
                    )}
